...
After a shot is detected the elapsed time between that current shot and the last shot is stored. The timer then resets to count the time between shots.
...
Switch up and down. This works in most modes. Select it again if you fucked up.
The tilt sensor has a short debounce set so when the badge is flapping around your neck the display doesn’t keep flipping back and forth. 
### 9. “thresh” = Microphone Threshold Adjustment
To give everyone the most control you can set the raw microphone ADC threshold.
The select button cycles through available values (step 20) with the highest value being “off” (no shots will be counted). The decimal point is lit on the currently selected value.
...
You can choose between “High” (default), “med” (medium), and “dim”. However I recommend you keep the badge in “High” mode when using the microphone, there seems to be something causing the mic ADC to read higher when digits aren’t lit at max brightness. 
...
Each badge read slightly differently, so there is an offset available to calibrate the temp 
...
The decimal point pin can be hooked to an external relay. Due to current sinking on the cathode size of the display, the decimal points are lit the opposite of what you might expect.
